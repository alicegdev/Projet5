...
let proverbesConfuceens = new Generateur(
    ...
    [
        "même les plus fous.",
        "le plus vil des crapauds.",
        "la plus haute montagne.",
        "le doute le plus abyssal.",
    ]
);

let burgerQuiz = new Generateur(
    [
        "Qui est le plus vieux :  ",
        "Questionnaire de rapidité : ",
        "Qu'est-ce qui est le plus bruyant : ",
        "Qui est le plus fort en maths :  ",
    ],
    ["Yoda ou ", "Albator ou ", "vos voisins ou ", "une perceuse ou "],
    [
        "Louis XIV ? ",
        "Didier, le chien joué par Alain Chabat ? ",
        "la SNCF ? ",
        "Matt Pokora ? ",
    ]
);
 ...
 */
function ecrireDansLEspaceCitations(espaceCitations, fois, Generateur) {
...
function afficher() {
    ...
    let fois = prompt(
        "Combien de citations ou de questions souhaitez-vous voir ?"
    );
    if (parseInt(fois) > 5 || parseInt(fois) <= 0 || isNaN(fois) == true) {
        ...
        switch (parseInt(choix)) {
            ...
            case 2:
                ecrireDansLEspaceCitations(espaceCitations, parseInt(fois), burgerQuiz);
                break;
